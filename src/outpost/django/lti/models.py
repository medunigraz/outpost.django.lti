...
class History(models.Model):
    ...
    token = models.CharField(
        max_length=max(settings.LTI_TOKEN_LENGTH), blank=True, null=True
    )

    class Meta:
        indexes = (models.Index(fields=["consumer", "timestamp", "nonce", "token"]),)
...
class DebugResource(Resource):
    ...
    def render(self, user, params):
        data = {
            ...
            "json": highlight(
                json.dumps(dict(params), indent=2), JsonLexer(), self.formater
            ),
        }
